...
def create_matrix(size: int) -> np.ndarray:
    ...
    if size > 64:
        matrix[
            10:18, 7 * size // 8 - 4 : 7 * size // 8, 7 * size // 8 - 4 : 7 * size // 8
        ] = 1
...
def run_benchmark(sizes=None, runs=10):
    ...
    if sizes is None:
        sizes = [32, 64, 128, 256, 512, 1024, 120]
    ...
    for size in sizes:
        ...
        matrix_torch = torch.as_tensor(matrix, dtype=torch.float32).to(DEVICE)
        matrix_cupy = cp.asarray(matrix)  # Pre-allocate on GPU for fair comparison
        ...
        results["cupy_ndimage"].append(safe_benchmark(cupy_ndimage_cc, matrix_cupy))
        results["cucim"].append(safe_benchmark(cucim_cc, matrix_cupy))
...
def plot_results(results, output_dir=None):
    ...
    plt.plot(
        sizes,
        results["torch_nosync"],
        marker="s",
        linestyle="--",
        label="PyTorch no sync (GPU, 3D)",
    )
    ...
    plt.savefig(
        output_dir / "connected_components_comparison_line_graph.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
    ...
    # GPU methods (top row)
    axes[0, 0].plot(
        sizes, results["torch"], marker="s", color="orange", label="PyTorch"
    )
    axes[0, 0].plot(
        sizes,
        results["torch_nosync"],
    ...
    axes[0, 1].plot(
        sizes, results["cupy_ndimage"], marker="d", color="purple", label="CuPy ndimage"
    )
    ...
    axes[1, 0].plot(
        sizes, results["skimage"], marker="o", color="blue", label="skimage"
    )
    ...
    axes[1, 1].plot(
        sizes, results["simpleitk"], marker="v", color="teal", label="SimpleITK"
    )
    axes[1, 1].plot(
        sizes, results["mahotas"], marker="p", color="brown", label="Mahotas"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "connected_components_comparison_separate_plots.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.close()
